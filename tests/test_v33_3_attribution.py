...
def test_all_agents_are_attributed_and_missing_agents_are_explicit():
    ...
    rows = ledger.record_council(
        ...
        council={"agents": [
            {"agent": "Q1", "decision": "BIAS_UP", "confidence": 0.8, "evidence_count": 4},
            {"agent": "Q2", "decision": "BIAS_DOWN", "confidence": 0.2, "evidence_count": 2},
        ]},
    )
    assert len(rows) == 8
...
def test_settlement_updates_all_agent_records_for_one_council():
    ledger = AgentEvidenceAttribution()
    ledger.record_council(
        asset="XAUUSD", timeframe="M15", timestamp="2026-09-15T20:00:00Z",
        council={"agents": [
            {"agent": "Q1", "decision": "BIAS_UP", "confidence": 1.0, "evidence_count": 3},
            {"agent": "Q2", "decision": "BIAS_DOWN", "confidence": 1.0, "evidence_count": 3},
        ]},
    )
    assert ledger.settle(asset="XAUUSD", observation_timestamp="2026-09-15T20:00:00Z",
                         outcome_r=1.0, mae_r=-0.2, mfe_r=1.4) == 8
    assert ledger.summary("Q1")["accuracy"] == 1.0
    assert ledger.summary("Q2")["accuracy"] == 0.0
