...
def obs(i: int, asset: str = "XAUUSD", regime: str = "TREND") -> OOSObservation:
    return OOSObservation(timestamp_utc=(BASE + timedelta(hours=i)).isoformat(), asset=asset, timeframe="1H", regime=regime, agent="Q1", confidence=0.8 if i % 2 == 0 else 0.3, correct=i % 2 == 0, realized_r=1.0 if i % 3 else -0.5)
